...
<html>
    <form action="./evnt006.jsp" method="post"><!mantem na mesma pagina e gera o request nos botoes)>
        <head>
...
<% if(null != request.getParameter("Home")){ 
    ...
    pageContext.forward("./Federecao.jsp");
            ...
            <input type="submit" name="aprovar" class="botao3" value="Aprovas Atletas"><br><br>
            <input type="submit" name="cancelar" class="botao3" value="Cancelar"><br><br>
<%}

else if(null != request.getParameter("aprovar")){                             //Terceira tela, quando aperta o botao inscrever. Deve aparecer os atletas
            transacoes.EVNT tn = new transacoes.EVNT();           //Cria uma transacao auxiliar
            ...
            for(int i=0; i<lista_atletas.size(); i++){                          //Percorre todo vetor
                ...
                if(ListaAtletas.getInscricao_Aprovada() == false){    %>
                    <tr>
                        <td>
                        <div style="float:left;width:25%;">
                        ...
                        <div style="float:left;width:25%;">
                            <input class="d" type="checkbox" name="<%= Atleta.getID_ATLETA() %>" value="<%=Atleta.getID_ATLETA()%>">
                        </div>
                        </td>
                    </tr>  
            ...
            for(int i = 0; i < lista_atletas.size(); i++){                      //Percorre vetor
                ListaAtletasDO ListaAtletas = (ListaAtletasDO)lista_atletas.elementAt(i);//Cria os objetos Lista atleta
                AtletaDO Atleta = tn.busca_atleta_por_ID_atleta(ListaAtletas.getId_ATLETA());//Cria os objetos atleta
                if(null != request.getParameter(Integer.toString(Atleta.getID_ATLETA()))){
                //Caso o checkbox daquele atleta for selecionado
                    ListaAtletas.setInscricao_Aprovada(true);
                    if(tn.atualizar_inscricao_ListaAtletas(ListaAtletas) != true){
            ...
            <h3>Houve um erro na aprovacao dos seguintes atletas</h3>
            <%for(int i = 0; i < nao_atualizado.size(); i++){%>
                <%= nao_atualizado.elementAt(i)%><br>
            <%}
            }
            else{%>
            <h3>Os seguintes atletas foram aprovados com sucesso</h3>
