...
pageContext.forward("./index.jsp");
    ...
    Vector atletas = tn.vetor_troca_atletas_clube(id_user);
    if (atletas.size() == 0) {
        %>Nenhuma troca<%
    }
...
%>
                    <table>
                        <br>
                        <text>Atleta: <%=atl.getNOME_ATLETA()%> <br>Requerente: <%=clb.getNomeClube()%></text>
                        <a href=tatl003.jsp?action=showApprovalForm&id_atl=<%=atl.getID_ATLETA()%>>Aprovar</a>
                        <a href=tatl003.jsp?action=showRepprovalForm&id_atl=<%=atl.getID_ATLETA()%>>Reprovar</a>
                        <br>
                    </table>
<%    }
    }
    if (action.equals("showApprovalForm")) {
        ...
        try {
            result = tn.aprovar_troca_clube_atleta(id_atleta);
        } catch (Exception e) {
%>           <%= e.toString() %>
<%
       }
...
<%     }
    ...
    if (action.equals("showRepprovalForm")) {
        ...
        boolean result = false;
        try {
            result = tn.reprovar_troca_clube_atleta(id_atleta);
        } catch (Exception e) {
%>           <%= e.toString() %>
<%
       }
